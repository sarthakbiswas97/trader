...
export default function PredictionsPage() {
  ...
  const handleGenerate = () => {
    ...
    es.addEventListener("error", () => {
      // EventSource auto-reconnects on error; we just stop
      stopStream();
    });
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            ML-powered price direction predictions for NIFTY 50
          </p>
        </div>
        <button
      ...
      {!streaming && predictions.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground/50">
          ...
          <p className="text-xs mt-1">
            Click &quot;Generate Predictions&quot; to stream-analyze all 48
            NIFTY 50 stocks
          </p>
        </div>
      )}

      {/* Prediction Cards — grouped by direction */}
